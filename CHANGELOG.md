...
- **macOS can reach Discord's RPC socket**: the IPC candidates now include `~/Library/Application Support/discord/sock` (where the macOS Discord client exposes its socket), with `/tmp/discord-ipc-0` kept as a fallback for older builds. Previously macOS only probed `/tmp/discord-ipc-0`, so every quest failed to connect.
...
### Fixed
- **Honest quest progress**: the Endfield quest no longer ships with a hardcoded 79% progress value — every quest now starts at 0% (the old value was leftover demo data; the app never persists progress)
- **Brand mark**: the header logo now renders the flat blurple **star** from the design contract instead of accidentally reusing the GitHub icon (which looked like a GitHub cat overlapping the logo)
- **Update check (403)**: the GitHub latest-release API rejects requests without a `User-Agent`, and reqwest sends none by default — `check_for_update` now sends `User-Agent: astral/<version>`, so the header pill reports "Up to date" instead of "Check failed"
- **External links**: added a `shell:allow-open` capability (missing before, so `open` was denied) and an `on_navigation` guard so the GitHub link always opens in the system browser — the webview never navigates to an external site

### Honest completion & no-ghost activity (Direction A)
...
### Update check, GitHub link & README rewrite
...
- Header now shows a check-for-updates pill (idle → checking → up to date / new version available with a link to the release page) plus a GitHub logo that opens the repository via the shell plugin
...
[2.11.0]: https://github.com/nguyenthanhthe/astral/releases/tag/v2.11.0
